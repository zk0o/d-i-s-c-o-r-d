...
if (this.logs.length >
200) this.logs = this.logs.slice(0, 200);
...
// Desktop tasks only work on current account
...
.log-info .log-message { color: #00aff4; }
.log-success .log-message { color: #43b581; }
.log-warning .log-message { color: #faa61a; }
.log-error .log-message { color: #f04747; }
.input-group {
display: flex;
gap: 10px;
margin-bottom: 16px;
...
<div id="quest-pro-header">
  ...
  <div class="header-controls">
    <button class="minimize-btn" onclick="document.getElementById('quest-pro-ui').style.display='none'">
      −
    </button>
    <button class="close-btn" id="quest-pro-close">
      ×
    </button>
  </div>
</div>
...
<div class="stats-grid">
  ...
  <div class="stat-card">
    <h3>
      ${Array.from(questManager.activeQuests.values()).reduce((a,b) =>
      a + b.length, 0)}
    </h3>
    <p>
      🎯 Active Quests
...
<div id="recent-logs">
  ...
  <p>
    Start completing quests to see activity here
  </p>
</div>
'}
</div>
...
<div class="section-header">
  <h3>
    Accounts (${accountManager.accounts.length})
  </h3>
...
<div id="accounts-list">
  ...
  <div class="empty-state">
    ...
    <p>
      Add your first account token above to get started
    </p>
  </div>
  ...
  <div class="account-card ${isCurrent ? 'current' : ''}">
    ...
    <div class="account-info">
      <h4>
        ${acc.username || 'Loading...'}${acc.discriminator ? '#' + acc.discriminator : ''}
      </h4>
      <p>
        ${quests.length} active quest${quests.length !== 1 ? 's' : ''}
        ${isCurrent ? ' • <span class="badge badge-success">
        Current
      </span>
      ' : ''}
    </p>
...
<div class="section-header">
  <h3>
    All Quests (${allQuests.reduce((a,b) =>
    a + b[1].length, 0)})
  </h3>
...
<div class="empty-state">
  ...
  <p>
    Load quests from your accounts in the Accounts tab
  </p>
</div>
` : allQuests.map(([accountId, quests]) =>
{
const account = accountManager.accounts.find(a =>
a.id === accountId);
return `
...
<div style="background: #383a40; padding: 20px; border-radius: 8px; margin-bottom: 16px;">
  <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
    <div>
      ...
      <p style="margin: 0; font-size: 13px; color: #b5bac1;">
        Automatically complete quests when loaded
      </p>
    </div>
...
<div style="background: #383a40; padding: 20px; border-radius: 8px; margin-bottom: 16px;">
  ...
  <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; font-size: 13px;">
    ...
    <div>
      ...
      <strong style="float: right; color: #43b581;">
        ${questManager.stats.completed}
      </strong>
    </div>
    <div>
      ...
      <strong style="float: right; color: #f04747;">
        ${questManager.stats.failed}
      </strong>
    </div>
    <div>
      <span style="color: #b5bac1;">
      ...
      <strong style="float: right; color: #5865f2;">
        ${questManager.stats.rewards}
      </strong>
    </div>
  </div>
...
})();-primary" onclick="window.refreshAllQuests()" style="flex: 1;">
🔄 Refresh All Quests
</button>
